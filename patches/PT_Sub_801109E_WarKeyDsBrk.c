// @Description PT_Sub_801109E_WarKeyDsBrk
...
int PT_Sub_801109E_WarKeyDsBrk()
{
	// Vanilla code
	int v0;
	int v1;
	...
	if (Wario_usMukiX & 0x10)
	{
		...
		{
			// Custom Code
			if (KeyPressContinuous[0] & 0x20 && dashAttackFlag == 1)
			{ // Hold Left
				Wario_usMukiX = 0x20;
	...
	else
	{
		...
		{
			// Custom Code
			if (KeyPressContinuous[0] & 0x10 && dashAttackFlag == 1)
			{ // Hold Right
				Wario_usMukiX = 0x10;
				Wario_sMvSpeedX = MAX_SPEED_X;
				return DATTACK;
