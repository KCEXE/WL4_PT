// @Description PT_Sub_8011590_WarKeyDAttack
// @HookAddress 0x2DEE1C
// @HookString P
// Mode: Thumb
// Made by beco

#define MAX_SPEED_X 128
...
#define Wario_ucStat (*(volatile unsigned char *)0x3001898)
...
#define WarBk_ucStat (*(volatile unsigned short *)0x30018D5)
...
int PT_Sub_8011590_WarKeyDAttack()
{
	short v1;
	short v2;

	if (usTrg_KeyPress1Frame[0] & KEY_A) // press A
	{
		Wario_ucJpFlg = 1;
		return 254;
	}
	...
	{
		Sub_8088620_WarioVoiceSet(WV_HAPPY_2);
		Wario_ucReact = 7; // Trans form into spring Wario 
		return 4; // Wario_ucStat = SPRING_DJUMPB
	}
	if (KeyPressContinuous[0] & KEY_DOWN) // press Down
	{
		return ROLL; // Change to ROLL
	}
	if (KeyPressContinuous[0] & Wario_usMukiX) // hold left or right (Wario's direction)
	{
