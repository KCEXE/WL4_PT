// @Description PT_Sub_8011280_WarKeyRoll
...
int PT_Sub_8011280_WarKeyRoll()
{
	...
	// Keep rolling during holding down
	if (KeyPressContinuous[0] & KEY_DOWN)
	{
		...
		// Cancel rolling if press opposite direction
		if (KeyPressContinuous[0] & (Wario_usMukiX ^ 0x30))
		{
			return DSBRK;
		}
	}
	...
	{ // When releasing down
		if (Sub_8014090_WarHeadCheck(0, 0xFF84, 1) == 0) // Keep rolling if ceiling is too close
		{
			if (KeyPressContinuous[0] & 0x300) // hold L or R
			{
				if (Wario_sMvSpeedX >= MAX_SPEED_X)
				{
					return DATTACK;
				}
			}
